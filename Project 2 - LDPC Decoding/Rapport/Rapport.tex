...
As it is a BSC channel it is sufficient to consider the all zero codeword as the swap is independent of the bit (0 or 1) so only considering this word is sufficient to quantize the performance of our decoder.
...
Then as $h\in C^2$ the recurrence sequence $\{p\}_{n\in\mathbb{N}}$ converges if and only if there exist $l$ so that $h'(x)\leq l <1$ and then the sequence converges to the unique fixed point of h (in our case 0). %Je crois que c'est pas le bon théorème, son critère est pas nécéssaire je crois
